...
const DeliveredPage = () => {
  const { data: deliveredOrders = [], refetch , isLoading } = useOrders();
  ...
  // Filter and group delivered orders with search functionality
  const filteredOrders = deliveredOrders.flatMap((customerOrder) =>
    customerOrder.orders
      .filter(
        (order) => order.status === true && order.order_id.includes(searchTerm) // Filter by order ID using search term
      )
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-semibold mb-4">Delivered Orders</h1>
      <Toaster />
      ...
      <div className="mb-4">
        <span className="text-lg font-semibold">
          Total Delivered Orders : {filteredOrders.length}
        </span>
      </div>
      {/* Orders Table */}
